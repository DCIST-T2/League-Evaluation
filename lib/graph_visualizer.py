...
try:
    from core import *
    from game_utils import read_yml_file, extract_positions_from_config
except ImportError:
    from lib.core import *
    from lib.game_utils import read_yml_file, extract_positions_from_config
...
class GraphVisualizer:
            ...
            with open(file_path, "rb") as gf:
                try:
                    ...
                    if file_path.endswith(".yml"):
                        ...
                        root_dir = os.path.dirname(os.path.dirname(file_path))  # Go up two levels: from config file to project root
                        graph_file_path = os.path.join(root_dir, "graphs", graph_name)
    ...
    def color_nodes(self, node_list: list[int], color: str, mode: str = "solid", name: Optional[str] = None, size_multiplier: float = 1.0) -> None:
        ...
        for node in node_list:
            if node in self.graph.nodes():
                ...
                self.node_transparency.setdefault(node, []).append(0.4 if mode == "transparent" else 1.0)
    ...
    def _visualize_static(self, save_path: Optional[str] = None, transparent_background: bool = False, quick: Optional[bool] = False) -> None:
        ...
        if quick:
            node_sizes = [s * 0.5 for s in node_sizes]
    ...
    def _display_legend(self, screen: pygame.Surface, legend_font: pygame.font.Font, screen_size: Tuple[int, int]) -> None:
        ...
        for i, (group_name, color, mode) in enumerate(legend_items):
            ...
            # Draw color circle
            r, g, b, a = to_rgba(color, alpha=1.0 if mode == "solid" else 0.4)
...
if __name__ == "__main__":
    ...
    visualizer = GraphVisualizer(file_path=config_file_path, mode="interactive", simple_layout=False, debug=True)
